...
class ExcelExporter {
  // ============================================================
  // CONSOLIDAR MATERIALES (SUMA AUTOMÁTICA DE CANTIDADES)
  // ============================================================
  ...
  static Future<String> exportarMultiplesPisosExcel({
    ...
    for (final entry in solicitudesPorPiso.entries) {
      ...
      // Sanitizar nombre de hoja para Excel (máx 31 caracteres, sin caracteres especiales)
    ...
    final filePath = '${directory.path}/Cotizacion_Pisos_$timestamp.xlsx';
  ...
  static Future<String> exportarPisoIndividualExcel({
    ...
    final filePath = '${directory.path}/Cotizacion_${cleanPiso}_$timestamp.xlsx';
  ...
  // ============================================================
  // LLENAR HOJA DE CÁLCULO ESTILIZADA
  // ============================================================
  static void _llenarHojaPiso({
    ...
    sheet.cell(CellIndex.indexByString('A1')).value = TextCellValue('HOJA DE COTIZACIÓN - CONSOLIDADO DE MATERIALES');
    sheet.cell(CellIndex.indexByString('A1')).cellStyle = titleStyle;

    sheet.cell(CellIndex.indexByString('A3')).value = TextCellValue('Obra:');
    sheet.cell(CellIndex.indexByString('A3')).cellStyle = boldLabelStyle;
    sheet.cell(CellIndex.indexByString('B3')).value = TextCellValue(nombreObra ?? 'Obra General');

    sheet.cell(CellIndex.indexByString('A4')).value = TextCellValue('Piso / Nivel:');
    sheet.cell(CellIndex.indexByString('A4')).cellStyle = boldLabelStyle;
    sheet.cell(CellIndex.indexByString('B4')).value = TextCellValue(nombrePiso);

    final now = DateTime.now();
    sheet.cell(CellIndex.indexByString('A5')).value = TextCellValue('Fecha de Emisión:');
    sheet.cell(CellIndex.indexByString('A5')).cellStyle = boldLabelStyle;
    sheet.cell(CellIndex.indexByString('B5')).value = TextCellValue('${now.day}/${now.month}/${now.year}');

    final idsSolicitudes = solicitudes.map((s) => '#${s.idSolicitud}').join(', ');
    sheet.cell(CellIndex.indexByString('D3')).value = TextCellValue('Solicitudes Incluidas:');
    sheet.cell(CellIndex.indexByString('D3')).cellStyle = boldLabelStyle;
    sheet.cell(CellIndex.indexByString('E3')).value = TextCellValue(idsSolicitudes);

    sheet.cell(CellIndex.indexByString('D4')).value = TextCellValue('Total Órdenes:');
    sheet.cell(CellIndex.indexByString('D4')).cellStyle = boldLabelStyle;
    sheet.cell(CellIndex.indexByString('E4')).value = IntCellValue(solicitudes.length);
    ...
    // 3. LLENADO DE MATERIALES CONSOLIDADOS (CON SUMA AUTOMÁTICA)
    ...
    for (int i = 0; i < materialesConsolidados.length; i++) {
      ...
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: rowIndex)).value = TextCellValue(item.codigo);
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: rowIndex)).value = TextCellValue(item.nombre);

      final cellCant = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: rowIndex));
      cellCant.value = IntCellValue(item.cantidadTotal);
      cellCant.cellStyle = rowHighlightStyle;

      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: rowIndex)).value = TextCellValue('unid.');
      // Espacios en blanco para que el proveedor anote los precios
